...
for k, strat in enumerate(['S1', 'S2', 'S3', 'S4']):
    for j, ds in enumerate(datasets):
        ...
        for i, method in enumerate(['sar-em', 'lbe', 'pglin', 'pusb', 'threshold']):
            res[k, j, i] = np.mean(np.loadtxt('results_UCI/results_method_{}_label_scheme_{}_classifier_logistic_ds_{}.txt'.format(method, strat, ds.split('.')[0]))[:,7])

table_mean = np.mean(res, axis=0)
table_std = np.std(res, axis=0)
for j, ds in enumerate(datasets):
    table_values = list(map(lambda x, y: '{} & {}'.format(x, y), np.round(table_mean[j], 2), np.round(table_std[j], 2)))
